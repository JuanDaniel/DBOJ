 ...
 * @version	$Id$
 */
class XMPP extends XMPPHP_XMLStream {
	/**
	 * @var string
	 */
	public $server;
	...
	public function __construct($host, $port, $user, $password, $resource, $server = null, $printlog = false, $loglevel = null) {
        ...
        $this->addXPathHandler('{jabber:client}iq', 'roster_iq_handler');
        $this->addXPathHandler('{jabber:iq:roster}query', 'roster_iq_handler');
	}
...
//        $msg = 'Mensaje Recibido: '.strval($payload['body']);
//        $this->message($jid[0],$msg);
	}
	...
	*/
	public function getRoster() {
		$id = $this->getID();
//        $iq = "<iq from=\"$this->fulljid\" type=\"get\" id=\"$id\" to=\"$this->basejid\"><query xmlns=\"jabber:iq:roster\" /></iq>";
        $iq = "<iq xmlns='jabber:client' type='get' id='$id'><query xmlns='jabber:iq:roster' /></iq>";
...
//            print_r($item->subs);
			...
			if ($item->name == 'item') {
				...
				$name = array_key_exists('name',$item->attrs)?$item->attrs['name']:$jid; //MAY
